...
print()
print("Human Readable Formated Output (Whole Yards, Whole Meters and Both)")
...
print()
print("All Distances (whole yards or meters)")
print("%-8s | %-8s | %-8s  | %-8s | %-8s | %-9s | %-8s |" %
      ("Range Y", "Range M", "Drop", "MOA", "Mils", "Time", "Velocity"))
print("%-8s | %-8s | %-8s  | %-8s | %-8s | %-9s | %-8s |" %
      ("(Yards)", "(Meters)", "(Inches)", "", "", "(Seconds)", "(fps)"))
for point in hold_overs.points:
    pi = '{:.3f}'.format(round(point.path_inches, 3))
    moac = '{:.2f}'.format(round(point.moa_correction, 2))
    milc = '{:.2f}'.format(round(point.mil_correction, 2))
    ...
    vel = '{:.2f}'.format(round(point.velocity, 2))
    print("%8s | %8s | %8s  | %8s | %8s | %9s | %8s |" %
          (point.yards, point.meters, pi, moac, milc, stime, vel))
